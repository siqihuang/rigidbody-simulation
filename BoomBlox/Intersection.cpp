...
namespace
{
	...
	{
		// TODO 
		Vector3 center=box->GetPosition();
		Vector3 halfVector=box->GetHalfSize();
		Vector3 p1=Vector3(halfVector.x,halfVector.y,halfVector.z);
		Vector3 p2=Vector3(halfVector.x,halfVector.y,-halfVector.z);
		Vector3 p3=Vector3(halfVector.x,-halfVector.y,halfVector.z);
		Vector3 p4=Vector3(halfVector.x,-halfVector.y,-halfVector.z);
		Vector3 p5=Vector3(-halfVector.x,halfVector.y,halfVector.z);
		Vector3 p6=Vector3(-halfVector.x,halfVector.y,-halfVector.z);
		Vector3 p7=Vector3(-halfVector.x,-halfVector.y,halfVector.z);
		Vector3 p8=Vector3(-halfVector.x,-halfVector.y,-halfVector.z);
		Vector4 p[8];
		p[0]=box->GetTransformation()*Vector4(p1);
		p[1]=box->GetTransformation()*Vector4(p2);
		p[2]=box->GetTransformation()*Vector4(p3);
		p[3]=box->GetTransformation()*Vector4(p4);
		p[4]=box->GetTransformation()*Vector4(p5);
		p[5]=box->GetTransformation()*Vector4(p6);
		p[6]=box->GetTransformation()*Vector4(p7);
		p[7]=box->GetTransformation()*Vector4(p8);

		int num[8];
		for(int i=0;i<8;i++) num[i]=i;
		for(int i=0;i<8;i++){
			for(int j=0;j<7-i;j++){
				if(p[num[j]].y>p[num[j+1]].y){
					int ntmp=num[j+1];
					num[j+1]=num[j];
					num[j]=ntmp;
				}
		...
		//for(int i=0;i<8;i++)
		//std::cout<<p[num[i]]<<std::endl;
		//then check
		if(p[num[0]].y<0){
			if(fabs(p[num[0]].y-p[num[3]].y)<1e-3){
				Vector4 v=p[num[0]]+p[num[1]]+p[num[2]]+p[num[3]];
				v=v/4;
				Vector3 v3(v.x,v.y,v.z);
				intersections.push_back(Intersection(box,ground,v3,Vector3(0,1,0)));
			}
			else if(fabs(p[num[0]].y-p[num[1]].y)<1e-3){
				Vector4 v=p[num[0]]+p[num[1]];
				v=v/2;
				Vector3 v3(v.x,v.y,v.z);
				intersections.push_back(Intersection(box,ground,v3,Vector3(0,1,0)));
			}
	...
	void FindIntersectionGroundSphere(Ground * ground, Sphere * sphere, std::vector<Intersection> & intersections)
	{
		...
		if(p.y<sphere->GetRadius()+1e-3){
			Vector3 interPoint=p;
			interPoint.y-=sphere->GetRadius();
			intersections.push_back(Intersection(sphere,ground,interPoint,Vector3(0,1,0)));
		}
	}
	...
	TEST(FindIntersectionSphereSphere, Intersection)
	{
		...
		std::cout<<i.back().outVector<<std::endl;
		CHECK(i.back().outVector.positionEquals(Vector3(1,0,0)));
	...
	void FindIntersectionSphereSphere(Sphere * sphere, Sphere * sphere2, std::vector<Intersection> & intersections)
	{
		// TODO
		Vector3 p=sphere->GetPosition();
		Vector3 p2=sphere2->GetPosition();
		float r=sphere->GetRadius();
		...
		if(distSq<(r+r2+1e-4)){
			...
			//Vector3 d1=(d+d2)/2;
			Vector3 vn=v.normalisedCopy();
			intersections.push_back(Intersection(sphere2,sphere,(p+p2)/2,vn));
		}
	}
